const { PREFIX, TOKEN, VER, LOOT, ENEMYDATA, MATCH, USERS, ENEMIES } = require('./config.json');
const fs = require('fs');
...
app.get('/respawn', async function(req, res){
		...
		if(!user){
				res.render(__dirname + '/views/error.ejs', {
						error: 'Sorry, I could not find that user in my database. Please try again or contact **ItzWiresDev#6193**.',
				})
				return;
		}
...
b.on('message', async function(msg){
		...
		if (Math.floor(Math.random() * 40) === 1){
				setTimeout(async function(){
						let enemy2 = Enemies[Math.floor((Math.random() * Enemies.length) + 0)]
